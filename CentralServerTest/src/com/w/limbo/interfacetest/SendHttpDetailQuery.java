...
public class SendHttpDetailQuery {
	...
	public static void sendQuery(HttpClient client) throws Exception {
		...
		for (int i = 0; i < 1; i++) {
			String msisdn = GetMsisdnFromFile.getInstance("msisdn.txt")
					.getMsisdnBySize(10);

			long starttime = System.currentTimeMillis();
			ContentResponse response = client
					.newRequest("https://localhost:8009/detailquery/")
					.param("token", "msisdn").param("querytype", "msisdn")
					.param("index", msisdn).send();
